...
class BatchInputGenerator:
    def __init__(self, chapters_dir='preprocessed_chapters', prompts_dir='prompts', output_file='openai_batch_input_file.jsonl'):
    ...
    def estimate_openai_cost(self, num_prompts, total_input_tokens, num_topics, prices):
        ...
        total_output_tokens = num_topics * 5 * 2000
        batch_discount = 0.5
        ...
        print("NOTE: Prompt caching does not apply as each topic generates a unique request.")
        print("-" * 35)
                ...
                for topic in chapter_data.get('TOPIC WISE SECTIONS', []):
                    ...
                    for prompt_name, prompt_template in prompt_templates.items():
                        if not prompt_template:
                            continue
                        user_content = prompt_template.replace('{{chapter_text}}', topic_text)
