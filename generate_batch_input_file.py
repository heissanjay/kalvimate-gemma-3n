...
class BatchInputGenerator:
    ...
    def estimate_openai_cost(self, num_prompts, total_input_tokens, num_topics, prices):
        total_output_tokens = num_topics * 6000
        batch_discount = 0.5
        input_cost = (total_input_tokens / 1_000_000) * prices["input"] * batch_discount
        output_cost = (
            (total_output_tokens / 1_000_000) * prices["output"] * batch_discount
        )
        ...
        print("NOTE: A 50% discount is applied to these prices for Batch API usage.")
        print(
            "NOTE: Prompt caching does not apply as each topic generates a unique request."
        )
        print("-" * 35)
        print(f"Total API requests in batch file: {num_prompts}")
        print(f"Total unique topics processed: {num_topics}")
    ...
    def generate(self):
        ...
        if not chapter_files or not prompt_files:
            ...
            return
        prompt_templates = {
            os.path.splitext(pf)[0]: self.read_txt_file(
            ...
            for chapter_file in chapter_files:
                ...
                for topic in chapter_data.get("TOPIC WISE SECTIONS", []):
                    ...
                    for prompt_name, prompt_template in prompt_templates.items():
                        ...
                        custom_id = f"chapter-{chapter_number}-topic-{topic_num_raw.replace('.', '_')}-{prompt_name}"
                        request_body = {
                            "model": "gpt-4o-mini",
        ...
        num_topics = self.total_requests / len(prompt_files) if prompt_files else 0
        self.estimate_openai_cost(
            self.total_requests, estimated_input_tokens, num_topics, gpt4o_mini_prices
        )
